...
const char* ssid = "KD Note";
const char* password = "12345678";

const char* host = "webapiyogesh.azurewebsites.net";
const int httpsPort = 80; 

WiFiClient client;
//Wifi Details Ends Here

//Trolley Reset or clear Items from buffer
int inPin = 2;  //Switch should be connected to Pin 2
...
void setup() {
  ...
  //Connect to RFID Starts From Here
  SPI.begin();
...
void loop() {
      ...
      // the time
      if (reading == HIGH && previous == LOW && millis() - customtime > debounce) {
        if (state == HIGH)
          state = LOW;
        else
          state = HIGH;
    ...
    if (status == MI_OK)
    {
      ...
      String productid = String(serNum[0]);
      // If Product Scanned then send the Id to Service
      String url = "/api/values/additem?trolleyId=" + String(trolleyId) + "&productid=" + productid;
      String line = PostDataToEndPoint(url);
      if (line == "\r") {
          //Blink LED to Green
        }
        else
        {
          //Blink LED to RED and signal user to Rescan
          }
    }
                //Serial.println(" ");
...
void ResetTrolley()
{
      //Seperate Service Endpoint to reset the Trolley items
      String url = "/api/Values/reset";
      String line = PostDataToEndPoint(url);
      if (line == "\r") {
          //Blink LED to Green
